...
import os, sys
from multi_cmd.rl_utils.multi_copg import MultiCoPG, MultiSimGD
...
env = ProbabilisticElectricityMarket(game_end_prob=0.2)
dtype = torch.float32

policies = [policy().to(device) for _ in range(3)]
critics = [critic().to(device) for _ in range(3)]
...
train_wrap = MultiCoPG(
    ...
    device=device,
    potential=potentials.squared_distance(1./1e-2)
)
