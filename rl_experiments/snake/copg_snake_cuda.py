...
for t_eps in range(1, num_episode+1):
    ...
    for j in range(batch_size):
        ...
        while (True):
            ...
            # Send observation to GPU, sample, then collect back to CPU.
            actions = []
            for i, p in enumerate(policy_list):
                obs_gpu = torch.tensor([obs[i]], device=device)
                action = Categorical(p(obs_gpu)).sample()
    ...
    print('finish_batch:', time.time() - batch_start_time)
